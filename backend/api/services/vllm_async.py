...
def normalize_messages(messages: List[dict]) -> List[dict]:
    ...
    for m in messages:
        ...
        for p in m.get("content", []):
            ...
            elif isinstance(p, dict) and p.get("type") in ("input_audio", "audio_url"):
                au = p.get("audio_url")
                if not au and isinstance(p.get("input_audio"), dict):
                    data = p["input_audio"].get("data")
                    fmt = str(p["input_audio"].get("format") or "wav").lower()
                    if data:
                        au = {"url": f"data:audio/{fmt};base64,{data}"}
                url = au if isinstance(au, str) else (au.get("url") if isinstance(au, dict) else None)
                parts.append({"type":"audio_url","audio_url":{"url":url}} if url else p)
            else:
                parts.append(p)
